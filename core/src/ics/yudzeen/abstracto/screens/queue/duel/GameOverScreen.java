...
class GameOverScreen extends AbstractoScreen {
    ...
    private void initChatBubble() {
        ...
        chatBubble.setPosition(GameConstants.WIDTH/4 + 60, GameConstants.HEIGHT/2 - 30 - 60);
    }

    private void initChatLabel() {
        String text = gameWin ? "You have defeated \nme, " + game.getGamePreferences().name + "!"
                : "Is that all you've \ngot, "+game.getGamePreferences().name+"?";
        chatLabel = LabelFactory.createLabel(text, assets.fonts.verdana_30);
        chatLabel.setPosition(280 + 325/2 - chatLabel.getWidth()/2, 210 + 180/2 - chatLabel.getHeight()/2);
    }
