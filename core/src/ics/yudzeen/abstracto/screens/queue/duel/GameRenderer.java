...
class GameRenderer {
    ...
    private ImageButton enqueueButton;
    private ImageButton dequeueButton;
    ...
    void buildStage(Stage stage) {
        ...
        stage.addActor(enqueueButton);
        stage.addActor(dequeueButton);
    ...
    private void initPushButton() {
        enqueueButton = ButtonFactory.createImageButton(assets.games.enqueue);
        enqueueButton.setPosition(GameConstants.WIDTH/4 - enqueueButton.getWidth()/2 + 20, 30);
        enqueueButton.addListener(new ClickListener() {
            @Override
    ...
    private void initPopButton() {
        dequeueButton = ButtonFactory.createImageButton(assets.games.dequeue);
        dequeueButton.setPosition(GameConstants.WIDTH*3/4 - dequeueButton.getWidth()/2 - 20, 30);
        dequeueButton.addListener(new ClickListener() {
            @Override
            public void clicked(InputEvent event, float x, float y) {
                onDequeuePressed();
            }
    ...
    private void initPortal() {
        portal = new Portal(gameScreen, gameController.enqueueDequeueList);
    ...
    private void initTooSlowImage() {
        ...
        tooSlowImage.setPosition(GameConstants.WIDTH - tooSlowImage.getWidth() - 50,
                enemyHPBar.getY() - tooSlowImage.getHeight() - 50);
    ...
    private void onEnqueuePressed() {
        if(gameController.onEnqueuePressed()) {
    ...
    private void onDequeuePressed() {
        if(gameController.onDequeuePressed()) {
            enemyHPBar.setCurrentHP(enemyHPBar.getCurrentHP() - 1);
        }
        else {
            playerHPBar.setCurrentHP(playerHPBar.getCurrentHP() - 1);
    ...
    void disablePushAndPopButtons(boolean disable) {
        if (disable) {
            enqueueButton.setTouchable(Touchable.disabled);
            dequeueButton.setTouchable(Touchable.disabled);
            dequeueButton.setDisabled(true);
            dequeueButton.setDisabled(true);
        }
        else {
            enqueueButton.setTouchable(Touchable.enabled);
            dequeueButton.setTouchable(Touchable.enabled);
            enqueueButton.setDisabled(false);
            dequeueButton.setDisabled(false);
        }
    }
